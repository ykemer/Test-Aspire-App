...
public class UnenrollStudentFromCourseHandler: IRequestHandler<UnenrollStudentFromCourseCommand, ErrorOr<Deleted>>
{
    ...
    public UnenrollStudentFromCourseHandler(ILogger<UnenrollStudentFromCourseHandler> logger, ApplicationDbContext dbContext, IMessageBusClient messageBusClient)
    {
        _logger = logger;
    ...
    public async Task<ErrorOr<Deleted>> Handle(UnenrollStudentFromCourseCommand command, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning("Enrollment for student {StudentId} and course {CourseId} doesn't exist", command.StudentId, command.CourseId);
