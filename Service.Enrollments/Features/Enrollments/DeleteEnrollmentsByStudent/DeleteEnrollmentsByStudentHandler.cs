...
public class DeleteEnrollmentsByStudentHandler: IRequestHandler<DeleteEnrollmentsByStudentCommand, ErrorOr<Deleted>>
{
    ...
    public DeleteEnrollmentsByStudentHandler(ApplicationDbContext dbContext, ILogger<DeleteEnrollmentsByStudentHandler> logger)
    {
        _dbContext = dbContext;
        _logger = logger;
    }

    public async Task<ErrorOr<Deleted>> Handle(DeleteEnrollmentsByStudentCommand request, CancellationToken cancellationToken)
    {
