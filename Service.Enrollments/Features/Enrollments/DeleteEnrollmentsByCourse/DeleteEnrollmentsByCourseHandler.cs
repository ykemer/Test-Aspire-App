...
public class DeleteEnrollmentsByCourseHandler: IRequestHandler<DeleteEnrollmentsByCourseCommand, ErrorOr<Deleted>>
{
    ...
    public DeleteEnrollmentsByCourseHandler(ApplicationDbContext dbContext, ILogger<DeleteEnrollmentsByCourseHandler> logger)
    {
        _dbContext = dbContext;
        _logger = logger;
    }

    public async Task<ErrorOr<Deleted>> Handle(DeleteEnrollmentsByCourseCommand request, CancellationToken cancellationToken)
    {
