...
public class GetCourseQueryHandlerTests
{
  ...
  public async Task Handle_ShouldReturnCourse_WhenCourseExists()
  {
    ...
    {
      CourseId = course.Id, RegistrationDeadline = now.AddDays(2), MaxStudents = 20, TotalStudents = 5
    });
  ...
  public async Task Handle_NotEnrolled_HasOpenWithCapacityClass_ShouldReturnCourse()
  {
    ...
    await _dbContext.Classes.AddRangeAsync(
      new Class
      {
        CourseId = course.Id, RegistrationDeadline = now.AddDays(1), MaxStudents = 10, TotalStudents = 5
      }, // open & has capacity
      new Class
      {
        CourseId = course.Id, RegistrationDeadline = now.AddDays(-1), MaxStudents = 10, TotalStudents = 10
      } // closed & full
    );
    await _dbContext.SaveChangesAsync();
