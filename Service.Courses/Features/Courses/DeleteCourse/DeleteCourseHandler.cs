...
public class DeleteCourseHandler: IRequestHandler<DeleteCourseCommand, ErrorOr<Deleted>>
{
    ...
    public DeleteCourseHandler(ApplicationDbContext dbContext, ILogger<DeleteCourseHandler> logger, IMessageBusClient messageBusClient)
    {
    ...
    public async Task<ErrorOr<Deleted>> Handle(DeleteCourseCommand request, CancellationToken cancellationToken)
    {
        ...
        if (course == null)
        {
            ...
            return Error.Unexpected("courses_service.delete_course.course_not_found", $"Course with id {request.Id} not found");
        }

        _dbContext.Courses.Remove(course);
        await _dbContext.SaveChangesAsync(cancellationToken);
        _messageBusClient.PublishCourseDeletedMessage(new CourseDeletedEvent
        {
            CourseId = course.Id  
        });
