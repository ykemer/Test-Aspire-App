...
public interface IClassesApiService
{
  ...
  Task<List<EnrollmentResponse>> GetClassEnrollments(Guid courseId, Guid classId,
    CancellationToken cancellationToken = default);
  ...
  Task UpdateClass(Guid courseId, Guid classId, UpdateClassRequest updateCourseRequest,
    CancellationToken cancellationToken = default);
}
