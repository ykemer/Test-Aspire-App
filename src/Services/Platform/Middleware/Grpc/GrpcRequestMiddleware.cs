...
public class GrpcRequestMiddleware : IGrpcRequestMiddleware
{
    ...
    public GrpcRequestMiddleware(ILogger<GrpcRequestMiddleware> logger)
    {
        _logger = logger;
    }

    public async Task<ErrorOr<TResponse>> SendGrpcRequestAsync<TResponse>(AsyncUnaryCall<TResponse> grpcCall, CancellationToken ct)
    {
        try
        {
