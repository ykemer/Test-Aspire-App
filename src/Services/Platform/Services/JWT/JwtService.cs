...
public class JwtService : IJwtService
{
  ...
  public async Task<JwtTokenServiceResponse> GenerateJwtToken(ApplicationUser user)
  {
    ...
    string? jwtToken = JwtBearer.CreateToken(
      o =>
      {
        ...
        o.Issuer = "https+http://platform"; //Environment.GetEnvironmentVariable("JWT_KEY_ISSUER");
        o.Audience = "https+http://platform"; //Environment.GetEnvironmentVariable("JWT_KEY_AUDIENCE");
  ...
  {
    string? secret = Environment.GetEnvironmentVariable("JWT_SIGN_KEY");
