...
public class CreateCourseHandler: IRequestHandler<CreateCourseCommand, ErrorOr<Course>>
{
    ...
    {
        var existingCourse = await _dbContext.Courses.FirstOrDefaultAsync(course => course.Name.ToLower() == request.Name.ToLower(), cancellationToken: cancellationToken);
        if(existingCourse != null)
        {
            _logger.LogWarning("Course with name {CourseName} already exists", request.Name);
            return Error.Conflict("courses_service.create_course.already_exists", description: $"Course {request.Name} already exists");
        }
