...
public class GetCourseEnrollmentsEndpoint : EndpointWithoutRequest<ErrorOr<List<EnrollmentResponse>>>
{
        ...
        CancellationToken ct)
    {
        ...
        var coursesRequest = _coursesGrpcService.GetCourseAsync(new GrpcGetCourseRequest
        {
            Id = id.ToString()
        }, cancellationToken: ct);
        ...
        var enrollmentsRequest = _enrollmentsGrpcService.GetCourseEnrollmentsAsync(new GrpcGetCourseEnrollmentsRequest
        {
            CourseId = course.Id
        });
